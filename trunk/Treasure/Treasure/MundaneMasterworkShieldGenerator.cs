...
{
	public class MundaneMasterworkShieldGenerator : BaseItemGenerator
	{
		public MundaneMasterworkShieldGenerator( TreasureDataContext dataContext ) : base(dataContext) { }

		public override void Generate( Collection<Item> items )
		{
			// Calculate item
			int roll = (int)Dice.Roll(100);
			var query = from a in m_DataContext.Mundane_ItemArmorMasterworkShields
						where a.Low <= roll &&
						a.High >= roll
						select a;
			var row = query.Single<Mundane_ItemArmorMasterworkShield>();
